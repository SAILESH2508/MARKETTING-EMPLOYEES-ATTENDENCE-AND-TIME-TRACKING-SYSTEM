...
try:
    from dashboard_modules.ui_helpers import (
        ...
        CustomButton, RoundedFrame, apply_treeview_style
    )
except ImportError:
...
# App theme colors from ui_helpers
BG = theme['bg']
FG = theme['fg']
ACCENT = theme['button']
...
plt.rcParams.update({
    'figure.facecolor': BG,
    'axes.facecolor': BG,
    'axes.edgecolor': theme['text_secondary'],
    'axes.labelcolor': FG,
    'xtick.color': FG,
    'ytick.color': FG,
    'text.color': FG,
    'legend.facecolor': BG,
    'legend.edgecolor': theme['text_secondary'],
...
class SalaryManager:
    ...
    def recalculate_net_salaries(self):
        ...
        for emp_id, salary, bonus, deductions in rows:
            try: s = float(salary or 0)
            except: s = 0.0
            try: b = float(bonus or 0)
            except: b = 0.0
            try: d = float(deductions or 0)
            except: d = 0.0
            net = s + b - d
    ...
    def build_ui(self):
        ...
        header.pack(pady=20)
        tk.Label(header, text="Salary Management", font=HEADER_FONT, bg=BG, fg=FG).pack()

        # Actions Grid
        action_frame = RoundedFrame(self.root, width=1200, height=100, bg_color=theme['card'])
        action_frame.pack(pady=10)
        inner_act = action_frame.inner_frame

        CustomButton(inner_act, text="🥧 View Pie Chart", command=self.draw_pie_chart, bg=theme['warning'], width=20).grid(row=0, column=0, padx=20, pady=20)
        CustomButton(inner_act, text="📊 View Bar Chart", command=self.draw_bar_chart, bg=theme['button'], width=20).grid(row=0, column=1, padx=20, pady=20)
        CustomButton(inner_act, text="📥 Export CSV", command=self.export_csv, bg=theme['success'], width=20).grid(row=0, column=2, padx=20, pady=20)
        CustomButton(inner_act, text="🏆 Top Earner", command=self.show_highest_paid, bg=theme['card_border'], fg=theme['fg'], width=20).grid(row=0, column=3, padx=20, pady=20)
    ...
    def build_form(self, parent):
        form_container = RoundedFrame(parent, width=400, height=500, bg_color=theme['card'])
        form_container.pack(side=LEFT, fill=Y, padx=10)
        f_inner = form_container.inner_frame

        tk.Label(f_inner, text="Edit Details", font=TITLE_FONT, bg=theme['card'], fg=theme['fg']).pack(pady=15)

        self.inputs = {}
        for field in ["Employee ID", "Name", "Salary", "Bonus", "Deductions"]:
            tk.Label(f_inner, text=field, font=SMALL_FONT, bg=theme['card'], fg=theme['text_secondary']).pack(anchor='w', padx=20, pady=(10,0))
            e = tk.Entry(f_inner, font=BODY_FONT, bg=theme['bg'], fg=theme['fg'], relief='flat')
        ...
        btn_box = tk.Frame(f_inner, bg=theme['card'])
        btn_box.pack(pady=20, fill='x', padx=20)
        CustomButton(btn_box, text="Update", command=self.update_salary, bg=theme['button'], width=10).pack(side=LEFT, padx=5)
        CustomButton(btn_box, text="Add", command=self.add_salary, bg=theme['success'], width=10).pack(side=LEFT, padx=5)
        CustomButton(btn_box, text="Clear", command=self.clear_entries, bg=theme['card_border'], fg=theme['fg'], width=8).pack(side=LEFT, padx=5)
    ...
    def build_table(self, parent):
        ...
        tk.Label(search_box, text="Search:", font=BODY_FONT, bg=BG, fg=FG).pack(side=LEFT)
        self.search_entry = tk.Entry(search_box, font=BODY_FONT)
        self.search_entry.pack(side=LEFT, padx=10)
        CustomButton(search_box, text="Go", command=self.search, bg=theme['button'], width=5).pack(side=LEFT)
        CustomButton(search_box, text="Show All", command=self.fetch_all, bg=theme['card_border'], fg=theme['fg'], width=10).pack(side=LEFT, padx=10)

        # Style for Treeview wrapper to handle scrollbars perfectly
        tree_frame = tk.Frame(table_container, bg=theme['card'])
    ...
    def add_salary(self):
        try:
            ...
            if not emp_id: return messagebox.showwarning("Input Required", "Employee ID is required.")
            name = self.name_entry.get().strip()
            if not name: return messagebox.showwarning("Input Required", "Name is required.")
            ...
            self.fetch_all()
            self.clear_entries()
        except sqlite3.IntegrityError: messagebox.showerror("Error", "Employee ID already exists.")
    ...
    def update_salary(self):
        try:
            ...
            if c.rowcount == 0: messagebox.showwarning("Not Found", "No record found.")
            else: messagebox.showinfo("Updated", "Salary record updated.")
            conn.commit()
            conn.close()
            self.fetch_all()
            self.clear_entries()
    ...
    def show_highest_paid(self):
        ...
        row = c.fetchone()
        conn.close()
        if row: messagebox.showinfo("Top Earner", f"ID: {row[0]}\nName: {row[1]}\nNet Salary: {self.format_currency(row[5])}")
        else: messagebox.showinfo("Top Earner", "No data available.")
    ...
    def on_row_select(self, event):
        ...
        if selected:
            ...
            if values:
                ...
                self.deductions_entry.insert(0, values[4].replace('₹', '').replace(',', ''))
    ...
    def draw_pie_chart(self):
        ...
        if not rows: return messagebox.showwarning("No Data", "No salary data available.")
        ...
        if total == 0: return messagebox.showwarning("No Data", "Net salaries are zero.")
    ...
    def draw_bar_chart(self):
        ...
        ax.barh(names, nets, color=theme['button'])
