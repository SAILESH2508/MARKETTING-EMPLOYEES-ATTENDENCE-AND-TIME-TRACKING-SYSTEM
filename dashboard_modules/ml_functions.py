...
def load_attendance_df(cursor):
    ...
    try:
        cursor.execute("SELECT emp_id, name, date, sign_in, sign_out FROM attendance WHERE sign_in IS NOT NULL")
    ...
    df['on_time'] = df['sign_in_seconds'].apply(lambda s: 1 if s is not None and s <= 9*3600 else 0)
...
def train_punctuality_model(cursor, return_eval=False, silent=True):
    ...
    if len(df) < 10:
        if not silent:
            messagebox.showinfo("ML", "Not enough data to train a decent model (need >=10 rows).")
        return None
    if y.nunique() < 2:
        if not silent:
            messagebox.showinfo("ML", "Target variable has no variation (everyone always on-time or always late). Cannot train model.")
        return None
    ...
    try:
        ...
        calib = CalibratedClassifierCV(LogisticRegression(max_iter=400), cv=5, method='sigmoid')
    ...
    if not silent:
        messagebox.showinfo("ML", f"Punctuality model trained. Test accuracy: {acc:.2f}")
    if return_eval:
        try:
...
def run_anomaly_scan(cursor, frame):
    ...
    clear_content(frame)
    tk.Label(frame, text="Anomaly Scan Results", font=("Arial", 14), bg=theme["bg"], fg=theme["fg"]).pack(pady=6)
    if anomalies.empty:
...
def train_salary_predictor(cursor, silent=True):
    ...
    if not rows or len(rows) < 5:
        if not silent:
            messagebox.showinfo("ML", "Not enough salary data to train (need >=5 rows).")
        return
    import pandas as pd
    ...
    if y.nunique() < 2:
        if not silent:
            messagebox.showinfo("ML", "Target salary has no variation (all salaries are the same). Cannot train model.")
        return
