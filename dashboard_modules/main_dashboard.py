...
from dashboard_modules.ui_helpers import (
    ...
    HEADER_FONT, SUBHEADER_FONT, TITLE_FONT, BODY_FONT, BUTTON_FONT,
    CustomButton, fade_in, update_theme, MODEL_DIR
)

# Import ML functions
try:
    from dashboard_modules.ml_functions import (
        get_ml_status, load_attendance_df,
        train_punctuality_model, train_anomaly_detector, train_salary_predictor,
        predict_punctuality_for_emp, predict_salary
    )
...
def employee_dashboard(root, emp_id, name, cursor, conn):
    ...
    tk.Label(name_frame, text=f"Welcome, {name}!", font=HEADER_FONT, 
            bg=theme["bg"], fg=theme["fg"]).pack(anchor='w')
    tk.Label(name_frame, text=f"ID: {emp_id}", font=TITLE_FONT, 
            bg=theme["bg"], fg=theme["button"]).pack(anchor='w')
    
    # Right side - Action buttons
    try:
        CustomButton(top_bar, text="🌗 Toggle Mode", command=toggle_theme_update).pack(side='right', padx=5)
        CustomButton(top_bar, text="🔄 Refresh", command=lambda: refresh_main_dashboard(content, emp_id, cursor)).pack(side='right', padx=5)
        CustomButton(top_bar, text="🚪 Logout", command=on_closing, bg=theme.get("error", "#F44336")).pack(side='right', padx=5)
    except Exception as e:
    ...
    try:
        CustomButton(action_bar, text="✅ Sign In", 
                     command=lambda: sign_in_fixed(emp_id, name, cursor, conn, content), 
                     bg=theme.get("success", "#4CAF50"), width=15).pack(side='left', padx=5)
        CustomButton(action_bar, text="❌ Sign Out", 
                     command=lambda: sign_out_fixed(emp_id, cursor, conn, content), 
                     bg=theme.get("error", "#F44336"), width=15).pack(side='left', padx=5)
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            tk.Label(tab_attendance, text=f"Attendance tab error: {e}", 
                    bg=theme["bg"], fg="red").pack(pady=20)
        
        # Analytics tab - create sub-notebook with error handling
        try:
            setup_analytics_tab(tab_analytics, emp_id, cursor)
        except Exception as e:
            tk.Label(tab_analytics, text=f"Analytics tab error: {e}", 
                    bg=theme["bg"], fg="red").pack(pady=20)
        ...
        try:
            ...
            tk.Label(tab_ml, text=f"ML tab error: {e}", 
                    bg=theme["bg"], fg="red").pack(pady=20)
            
    except Exception as e:
        # Fallback if notebook creation fails
        tk.Label(dashboard, text=f"Dashboard creation error: {e}", 
                bg=theme["bg"], fg="red", font=("Arial", 14)).pack(pady=50)

def setup_attendance_tab(tab_attendance, emp_id, cursor):
    """Setup attendance tab with error handling"""
    try:
        from dashboard_modules.chart_functions import show_heatmap, show_face_log, show_leaderboard
        ...
        try:
            ...
            tk.Label(heatmap_frame, text=f"Heatmap error: {e}", bg=theme["bg"], fg="red").pack()
            
        try:
            show_face_log(history_frame, emp_id, cursor)
        except Exception as e:
            tk.Label(history_frame, text=f"History error: {e}", bg=theme["bg"], fg="red").pack()
            
        try:
            show_leaderboard(leaderboard_frame, cursor)
        except Exception as e:
            tk.Label(leaderboard_frame, text=f"Leaderboard error: {e}", bg=theme["bg"], fg="red").pack()
            
    except ImportError as e:
        tk.Label(tab_attendance, text=f"Import error: {e}", bg=theme["bg"], fg="red").pack()

def setup_analytics_tab(tab_analytics, emp_id, cursor):
    """Setup analytics tab with error handling"""
    try:
        from dashboard_modules.analytics_functions import show_performance_trends, show_work_patterns, show_productivity_score
        from dashboard_modules.chart_functions import show_salary_pie, show_attendance_bar
        ...
        try:
            ...
            tk.Label(productivity_frame, text=f"Productivity error: {e}", bg=theme["bg"], fg="red").pack()
            
        try:
            show_performance_trends(performance_frame, emp_id, cursor)
        except Exception as e:
            tk.Label(performance_frame, text=f"Performance trends error: {e}", bg=theme["bg"], fg="red").pack()
            
        try:
            show_work_patterns(patterns_frame, emp_id, cursor)
        except Exception as e:
            tk.Label(patterns_frame, text=f"Work patterns error: {e}", bg=theme["bg"], fg="red").pack()
            
        try:
            show_salary_pie(salary_frame, emp_id, cursor)
        except Exception as e:
            tk.Label(salary_frame, text=f"Salary chart error: {e}", bg=theme["bg"], fg="red").pack()
            
        try:
            show_attendance_bar(ontime_frame, emp_id, cursor)
        except Exception as e:
            tk.Label(ontime_frame, text=f"Attendance bar error: {e}", bg=theme["bg"], fg="red").pack()
            
    except ImportError as e:
        tk.Label(tab_analytics, text=f"Analytics import error: {e}", bg=theme["bg"], fg="red").pack()
...
def setup_ml_tab(tab_ml, emp_id, cursor):
    ...
    try:
        ...
        tk.Label(title_frame, text="🤖 ML Insights & Analytics", font=HEADER_FONT, 
                bg=theme["bg"], fg=theme["fg"]).pack(anchor='w')
        
        if not SKLEARN_AVAILABLE:
            # Show installation instructions if ML libraries not available
            error_frame = tk.Frame(main_container, bg=theme["card"], relief='raised', bd=2)
            error_frame.pack(fill='both', expand=True, padx=10, pady=10)
            
            tk.Label(error_frame, text="❌ Machine Learning Libraries Not Available", 
                    font=("Arial", 14, "bold"), bg=theme["card"], fg="red").pack(pady=20)
            tk.Label(error_frame, text="To enable ML features, install required libraries:", 
                    bg=theme["card"], fg=theme["fg"]).pack(pady=10)
            tk.Label(error_frame, text="pip install scikit-learn pandas", 
                    font=("Courier", 12), bg=theme["card"], fg=theme["button"]).pack(pady=5)
            return
        
        # ML Status and Controls
        status_frame = tk.Frame(main_container, bg=theme["card"], relief='raised', bd=2)
        status_frame.pack(fill='x', pady=(0, 15))
        
        status_content = tk.Frame(status_frame, bg=theme["card"])
        status_content.pack(fill='x', padx=20, pady=15)
        
        tk.Label(status_content, text="✅ Machine Learning Ready", 
                font=("Arial", 12, "bold"), bg=theme["card"], fg="#4CAF50").pack(anchor='w')
        
        # Model status
        model_status = get_ml_status(cursor)
        tk.Label(status_content, text=model_status, 
                font=("Courier", 10), bg=theme["card"], fg=theme["fg"]).pack(anchor='w', pady=(5, 0))
        ...
        training_frame = tk.Frame(left_column, bg=theme["card"], relief='raised', bd=2)
        training_frame.pack(fill='x', pady=(0, 15))
        
        training_content = tk.Frame(training_frame, bg=theme["card"])
        training_content.pack(fill='x', padx=20, pady=15)
        
        tk.Label(training_content, text="🎯 Model Training", 
                font=("Arial", 12, "bold"), bg=theme["card"], fg=theme["fg"]).pack(anchor='w', pady=(0, 10))
        
        # Training buttons
        btn_frame1 = tk.Frame(training_content, bg=theme["card"])
        btn_frame1.pack(fill='x', pady=2)
        
        tk.Button(btn_frame1, text="Train Punctuality Model", 
                 command=lambda: train_punctuality_model(cursor, silent=False),
                 bg=theme["button"], fg="white", font=BUTTON_FONT, relief='flat').pack(side='left', padx=(0, 10))
        
        tk.Button(btn_frame1, text="Train Anomaly Detector", 
                 command=lambda: train_anomaly_detector(cursor, silent=False),
                 bg=theme["button"], fg="white", font=BUTTON_FONT, relief='flat').pack(side='left')
        
        btn_frame2 = tk.Frame(training_content, bg=theme["card"])
        btn_frame2.pack(fill='x', pady=2)
        
        tk.Button(btn_frame2, text="Train Salary Predictor", 
                 command=lambda: train_salary_predictor(cursor, silent=False),
                 bg=theme["button"], fg="white", font=BUTTON_FONT, relief='flat').pack(side='left', padx=(0, 10))
        
        tk.Button(btn_frame2, text="Train All Models", 
                 command=lambda: train_all_models(cursor),
                 bg=theme["hover"], fg="white", font=BUTTON_FONT, relief='flat').pack(side='left')
        
        # Predictions Section
        predictions_frame = tk.Frame(left_column, bg=theme["card"], relief='raised', bd=2)
        predictions_frame.pack(fill='both', expand=True)
        
        pred_content = tk.Frame(predictions_frame, bg=theme["card"])
        pred_content.pack(fill='both', expand=True, padx=20, pady=15)
        
        tk.Label(pred_content, text="🔮 Predictions & Analysis", 
                font=("Arial", 12, "bold"), bg=theme["card"], fg=theme["fg"]).pack(anchor='w', pady=(0, 10))
        ...
        tk.Button(pred_btn_frame1, text="Predict Punctuality", 
                 command=lambda: predict_punctuality_for_emp(emp_id, cursor),
                 bg="#4CAF50", fg="white", font=BUTTON_FONT, relief='flat').pack(side='left', padx=(0, 10))
        
        tk.Button(pred_btn_frame1, text="Predict Salary", 
                 command=lambda: predict_salary(emp_id, cursor),
                 bg="#4CAF50", fg="white", font=BUTTON_FONT, relief='flat').pack(side='left')
        ...
        anomaly_frame = tk.Frame(right_column, bg=theme["card"], relief='raised', bd=2)
        anomaly_frame.pack(fill='both', expand=True, pady=(0, 15))
        
        anomaly_content = tk.Frame(anomaly_frame, bg=theme["card"])
        anomaly_content.pack(fill='both', expand=True, padx=20, pady=15)
        
        tk.Label(anomaly_content, text="🚨 Anomaly Detection", 
                font=("Arial", 12, "bold"), bg=theme["card"], fg=theme["fg"]).pack(anchor='w', pady=(0, 10))
        
        # Anomaly scan button
        tk.Button(anomaly_content, text="Run Anomaly Scan", 
                 command=lambda: show_anomaly_results(anomaly_results_frame, cursor),
                 bg="#FF9800", fg="white", font=BUTTON_FONT, relief='flat').pack(anchor='w', pady=(0, 10))
        ...
        # ML Insights Section
        insights_frame = tk.Frame(right_column, bg=theme["card"], relief='raised', bd=2)
        insights_frame.pack(fill='both', expand=True)
        
        insights_content = tk.Frame(insights_frame, bg=theme["card"])
        insights_content.pack(fill='both', expand=True, padx=20, pady=15)
        
        tk.Label(insights_content, text="📊 ML Insights", 
                font=("Arial", 12, "bold"), bg=theme["card"], fg=theme["fg"]).pack(anchor='w', pady=(0, 10))
        ...
        error_label = tk.Label(tab_ml, text=f"ML tab error: {e}", 
                              bg=theme["bg"], fg="red", font=("Arial", 12))
...
def train_all_models(cursor):
    ...
    try:
        messagebox.showinfo("ML Training", "Training all models... This may take a moment.")
...
def show_anomaly_results(frame, cursor):
    ...
    try:
        ...
        if not os.path.exists(path):
            tk.Label(frame, text="❌ Anomaly model not found. Train it first.", 
                    bg=theme["card"], fg="red").pack(pady=10)
            return
        
        df, err = load_attendance_df(cursor)
        if err:
            tk.Label(frame, text=f"❌ Error: {err}", 
                    bg=theme["card"], fg="red").pack(pady=10)
            return
        ...
        if anomalies.empty:
            tk.Label(frame, text="✅ No anomalies detected", 
                    bg=theme["card"], fg="#4CAF50").pack(pady=10)
        else:
            tk.Label(frame, text=f"⚠️ Found {len(anomalies)} anomalies:", 
                    bg=theme["card"], fg="#FF9800").pack(pady=(0, 5))
            ...
            listbox = tk.Listbox(list_frame, yscrollcommand=scrollbar.set, 
                               bg=theme["card"], fg=theme["fg"], font=("Courier", 9))
            listbox.pack(fill='both', expand=True)
            scrollbar.config(command=listbox.yview)
            
            for _, row in anomalies.iterrows():
                listbox.insert('end', f"ID:{row['emp_id']} {row['date']} {row['sign_in']}")
...
def show_ml_insights(frame, emp_id, cursor):
    ...
    try:
        ...
        df, err = load_attendance_df(cursor)
        if err:
            tk.Label(frame, text=f"No data for insights: {err}", 
                    bg=theme["card"], fg=theme["fg"]).pack(pady=10)
            return
...
🎯 Model Status:"""
        ...
        tk.Label(frame, text=insights_text, 
                bg=theme["card"], fg=theme["fg"], font=("Courier", 10), 
                justify='left').pack(anchor='w', pady=10)
        
    except Exception as e:
        tk.Label(frame, text=f"Error generating insights: {e}", 
                bg=theme["card"], fg="red").pack(pady=10)
...
def show_main_dashboard_content(frame, emp_id, cursor):
    ...
    title_frame = tk.Frame(main_container, bg=theme['bg'])
    title_frame.pack(fill='x', pady=(0, 20))
    tk.Label(title_frame, text="📊 Dashboard Overview", font=HEADER_FONT, 
            bg=theme['bg'], fg=theme['fg']).pack(anchor='w')
...
def create_quick_stats_card(parent, row, col, emp_id, cursor):
    ...
    card_frame = tk.Frame(parent, bg=theme['card'], relief='raised', bd=2)
    card_frame.grid(row=row, column=col, sticky='nsew', padx=10, pady=10)
    
    # Card content
    content_frame = tk.Frame(card_frame, bg=theme['card'])
    content_frame.pack(fill='both', expand=True, padx=15, pady=15)
    
    tk.Label(content_frame, text="📈 Quick Stats", font=TITLE_FONT, 
            bg=theme['card'], fg=theme['fg']).pack(anchor='w', pady=(0, 10))
    
    try:
        # Get attendance count
        cursor.execute("SELECT COUNT(*) FROM attendance WHERE emp_id=? AND sign_in IS NOT NULL", (emp_id,))
        total_days = cursor.fetchone()[0]
        
        # Get on-time count
        cursor.execute("SELECT COUNT(*) FROM attendance WHERE emp_id=? AND TIME(sign_in) <= '09:00:00'", (emp_id,))
        ...
        tk.Label(stats_frame, text="Total Days:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        tk.Label(stats_frame, text=f"{total_days}", font=("Arial", 16, "bold"), 
                bg=theme['card'], fg=theme['button']).pack(anchor='w', pady=(0, 5))
        
        tk.Label(stats_frame, text="On-Time Rate:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        tk.Label(stats_frame, text=f"{on_time_pct:.1f}%", font=("Arial", 16, "bold"), 
                bg=theme['card'], fg=theme['success'] if on_time_pct >= 80 else theme['warning']).pack(anchor='w')
        
    except Exception as e:
        tk.Label(content_frame, text=f"Error loading stats: {e}", 
                bg=theme['card'], fg='red').pack()
...
def create_salary_summary_card(parent, row, col, emp_id, cursor):
    ...
    tk.Label(content_frame, text="💰 Salary Info", font=TITLE_FONT, 
            bg=theme['card'], fg=theme['fg']).pack(anchor='w', pady=(0, 10))
    ...
    try:
        ...
        if result:
            ...
            tk.Label(content_frame, text="Base Salary:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
            tk.Label(content_frame, text=f"₹{salary:,.0f}", font=("Arial", 14, "bold"), 
                    bg=theme['card'], fg=theme['button']).pack(anchor='w', pady=(0, 5))
            
            tk.Label(content_frame, text="Net Salary:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
            tk.Label(content_frame, text=f"₹{net_salary:,.0f}", font=("Arial", 14, "bold"), 
                    bg=theme['card'], fg=theme['success']).pack(anchor='w')
        else:
            tk.Label(content_frame, text="No salary data available", 
                    bg=theme['card'], fg=theme['fg']).pack()
            
    except Exception as e:
        tk.Label(content_frame, text=f"Error loading salary: {e}", 
                bg=theme['card'], fg='red').pack()
...
def create_status_card(parent, row, col, emp_id, cursor):
    ...
    tk.Label(content_frame, text="🕐 Today's Status", font=TITLE_FONT, 
            bg=theme['card'], fg=theme['fg']).pack(anchor='w', pady=(0, 10))
    ...
    try:
        ...
        if result and result[0]:
            ...
            tk.Label(content_frame, text="Sign In:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
            tk.Label(content_frame, text=sign_in_time, font=("Arial", 12, "bold"), 
                    bg=theme['card'], fg=theme['success']).pack(anchor='w', pady=(0, 5))
            
            if sign_out_time:
                tk.Label(content_frame, text="Sign Out:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
                tk.Label(content_frame, text=sign_out_time, font=("Arial", 12, "bold"), 
                        bg=theme['card'], fg=theme['warning']).pack(anchor='w')
                tk.Label(content_frame, text="✅ Day Complete", 
                        bg=theme['card'], fg=theme['success']).pack(anchor='w', pady=(5, 0))
            else:
                tk.Label(content_frame, text="🔄 Currently Working", 
                        bg=theme['card'], fg=theme['button']).pack(anchor='w', pady=(5, 0))
        else:
            tk.Label(content_frame, text="❌ Not signed in today", 
                    bg=theme['card'], fg=theme['warning']).pack()
            
    except Exception as e:
        tk.Label(content_frame, text=f"Error loading status: {e}", 
                bg=theme['card'], fg='red').pack()

def create_performance_card(parent, row, col, emp_id, cursor, colspan=1):
    """Create performance overview card"""
    card_frame = tk.Frame(parent, bg=theme['card'], relief='raised', bd=2)
    card_frame.grid(row=row, column=col, columnspan=colspan, sticky='nsew', padx=10, pady=10)
    
    content_frame = tk.Frame(card_frame, bg=theme['card'])
    content_frame.pack(fill='both', expand=True, padx=15, pady=15)
    
    tk.Label(content_frame, text="🎯 Performance Overview", font=TITLE_FONT, 
            bg=theme['card'], fg=theme['fg']).pack(anchor='w', pady=(0, 10))
    ...
    try:
        ...
        if result and result[0] > 0:
            ...
            tk.Label(punct_frame, text="Punctuality:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
            tk.Label(punct_frame, text=f"{punctuality:.1f}%", font=("Arial", 18, "bold"), 
                    bg=theme['card'], fg=theme['success'] if punctuality >= 90 else theme['warning']).pack(anchor='w')
            
            # Average hours
            hours_frame = tk.Frame(metrics_frame, bg=theme['card'])
            hours_frame.grid(row=0, column=1, sticky='w')
            tk.Label(hours_frame, text="Avg Hours/Day:", bg=theme['card'], fg=theme['fg']).pack(anchor='w')
            tk.Label(hours_frame, text=f"{avg_hours:.1f}h", font=("Arial", 18, "bold"), 
                    bg=theme['card'], fg=theme['button']).pack(anchor='w')
            
        else:
            tk.Label(content_frame, text="No performance data available yet", 
                    bg=theme['card'], fg=theme['fg']).pack()
            
    except Exception as e:
        tk.Label(content_frame, text=f"Error loading performance: {e}", 
                bg=theme['card'], fg='red').pack()
...
def create_recent_activity_card(parent, row, col, emp_id, cursor):
    ...
    tk.Label(content_frame, text="📋 Recent Activity", font=TITLE_FONT, 
            bg=theme['card'], fg=theme['fg']).pack(anchor='w', pady=(0, 10))
    
    try:
        cursor.execute("SELECT date, sign_in FROM attendance WHERE emp_id=? ORDER BY date DESC LIMIT 5", (emp_id,))
        ...
        if results:
            for date, sign_in in results:
                ...
                tk.Label(activity_frame, text=date, bg=theme['card'], fg=theme['fg'], 
                        font=("Arial", 9), width=12, anchor='w').pack(side='left')
                tk.Label(activity_frame, text=sign_in or "No sign-in", bg=theme['card'], 
                        fg=theme['fg'], font=("Arial", 9)).pack(side='left', padx=(10, 0))
        else:
            tk.Label(content_frame, text="No recent activity", 
                    bg=theme['card'], fg=theme['fg']).pack()
            
    except Exception as e:
        tk.Label(content_frame, text=f"Error loading activity: {e}", 
                bg=theme['card'], fg='red').pack()
...
def sign_out_fixed(emp_id, cursor, conn, content_frame):
    ...
    try:
        ...
        # Sign out
        cursor.execute("UPDATE attendance SET sign_out=? WHERE emp_id=? AND date=?", 
                      (time, emp_id, date))
        conn.commit()
