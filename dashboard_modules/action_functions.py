...
def sign_in(emp_id, name, cursor, conn):
    ...
    try:
        ...
        messagebox.showerror(
            "DB Error", "Database table 'attendance' not found. Please relaunch."
        )
        return
    if cursor.fetchone():
...
def export_attendance_csv(cursor):
    ...
    rows, err = get_db_data_safely(
        "SELECT emp_id, name, date, sign_in, sign_out FROM attendance", fetch_all=True
    )
    if err:
