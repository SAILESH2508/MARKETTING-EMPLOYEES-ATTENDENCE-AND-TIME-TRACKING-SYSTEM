...
try:
    ...
    theme = {"bg": "#1A1A2E", "fg": "white", "button": "#E94560", "hover": "#FF6F61", "card": "#2E2E2E"}
...
def show_performance_trends(frame, emp_id, cursor):
    ...
    # Title
    title_label = tk.Label(frame, text="📈 Performance Trends Analysis", 
    ...
    try:
        ...
        if not rows:
            tk.Label(frame, text="No attendance data found for this employee", 
                    bg=theme["bg"], fg=theme["fg"], font=("Arial", 12)).pack(pady=50)
            return
        
        if len(rows) < 3:
            tk.Label(frame, text=f"Need at least 3 records for trends (found {len(rows)})", 
                    bg=theme["bg"], fg=theme["fg"], font=("Arial", 12)).pack(pady=50)
            return
        ...
        if len(dates) < 3:
            tk.Label(frame, text="Not enough valid time data for analysis", 
                    bg=theme["bg"], fg=theme["fg"], font=("Arial", 12)).pack(pady=50)
            return
        
        # Create the chart with better spacing
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), facecolor=theme['bg'])
        fig.subplots_adjust(left=0.1, right=0.9, top=0.92, bottom=0.1, hspace=0.5)  # More space between subplots
        
        # Chart 1: Sign-in time trend
        ax1.set_facecolor(theme['card'])
        ax1.plot(dates, sign_in_hours, marker='o', color=theme['button'], 
                linewidth=2, markersize=6, label='Sign-in Time')
        ax1.axhline(y=9, color='red', linestyle='--', alpha=0.7, label='9 AM Threshold')
        ax1.set_title('Daily Sign-In Time Trend', color=theme['fg'], fontsize=14, pad=20)
        ...
        ax2.plot(dates, rolling_avg, marker='s', color=theme['hover'], 
                linewidth=2, markersize=6, label=f'{window_size}-Day Rolling Average')
        ax2.set_title('On-Time Performance Trend', color=theme['fg'], fontsize=14, pad=20)
        ...
        tk.Label(stats_frame, text="📊 Summary Statistics", 
                font=("Arial", 12, "bold"), bg=theme['card'], fg=theme['fg']).pack(pady=5)
        tk.Label(stats_frame, text=f"Average Arrival: {int(avg_time)}:{int((avg_time % 1) * 60):02d}", 
                bg=theme['card'], fg=theme['fg']).pack()
        tk.Label(stats_frame, text=f"On-Time Rate: {on_time_pct:.1f}%", 
                bg=theme['card'], fg=theme['fg']).pack()
        tk.Label(stats_frame, text=f"Total Records: {len(dates)}", 
                bg=theme['card'], fg=theme['fg']).pack(pady=(0,5))
        
        plt.close(fig)
        
    except Exception as e:
        error_label = tk.Label(frame, text=f"Error loading performance data: {str(e)}", 
...
def show_work_patterns(frame, emp_id, cursor):
    ...
    # Title
    title_label = tk.Label(frame, text="🧠 Work Pattern Analysis", 
                          font=("Arial", 16, "bold"), bg=theme["bg"], fg=theme["fg"])
    title_label.pack(pady=10)
    ...
    try:
        ...
        if len(rows) < 5:
            tk.Label(frame, text=f"Need at least 5 records for pattern analysis (found {len(rows)})", 
                    bg=theme["bg"], fg=theme["fg"], font=("Arial", 12)).pack(pady=50)
            return
        ...
        if day_stats:
            tk.Label(content_frame, text="📅 Performance by Day of Week", 
                    font=("Arial", 14, "bold"), bg=theme['card'], fg=theme['fg']).pack(anchor='w', pady=(0, 15))
            ...
            if day_rates:
                ...
                tk.Label(best_frame, text="✅ Best Day:", font=("Arial", 12, "bold"), 
                        bg=theme['card'], fg=theme['fg'], width=15, anchor='w').pack(side='left')
                tk.Label(best_frame, text=f"{best_day} ({day_rates[best_day]*100:.0f}% on-time)", 
                        bg=theme['card'], fg='#4CAF50', font=("Arial", 12)).pack(side='left', padx=10)
                
                # Worst day
                worst_frame = tk.Frame(content_frame, bg=theme['card'])
                worst_frame.pack(fill='x', pady=5)
                tk.Label(worst_frame, text="❌ Challenging Day:", font=("Arial", 12, "bold"), 
                        bg=theme['card'], fg=theme['fg'], width=15, anchor='w').pack(side='left')
                tk.Label(worst_frame, text=f"{worst_day} ({day_rates[worst_day]*100:.0f}% on-time)", 
                        bg=theme['card'], fg='#F44336', font=("Arial", 12)).pack(side='left', padx=10)
        ...
        if sign_in_times:
            ...
            std_dev = (sum((x - avg_arrival) ** 2 for x in sign_in_times) / len(sign_in_times)) ** 0.5
            ...
            tk.Label(arrival_frame, text="⏰ Average Arrival:", font=("Arial", 13, "bold"), 
                    bg=theme['card'], fg=theme['fg'], width=15, anchor='w').pack(side='left')
            tk.Label(arrival_frame, text=f"{int(avg_arrival)}:{int((avg_arrival % 1) * 60):02d}", 
                    font=("Arial", 13), bg=theme['card'], fg=theme['button']).pack(side='left', padx=10)
            
            # Consistency score
            consistency_frame = tk.Frame(content_frame, bg=theme['card'])
            consistency_frame.pack(fill='x', pady=10)
            tk.Label(consistency_frame, text="🎯 Consistency Score:", font=("Arial", 13, "bold"), 
                    bg=theme['card'], fg=theme['fg'], width=15, anchor='w').pack(side='left')
            consistency_color = '#4CAF50' if consistency >= 80 else '#FF9800' if consistency >= 60 else '#F44336'
            tk.Label(consistency_frame, text=f"{consistency:.1f}/100", 
                    font=("Arial", 13), bg=theme['card'], fg=consistency_color).pack(side='left', padx=10)
            
            # Punctuality rate
            punct_frame = tk.Frame(content_frame, bg=theme['card'])
            punct_frame.pack(fill='x', pady=10)
            tk.Label(punct_frame, text="📊 Punctuality Rate:", font=("Arial", 13, "bold"), 
                    bg=theme['card'], fg=theme['fg'], width=15, anchor='w').pack(side='left')
            punct_color = '#4CAF50' if punctuality_rate >= 90 else '#FF9800' if punctuality_rate >= 75 else '#F44336'
            tk.Label(punct_frame, text=f"{punctuality_rate:.1f}%", 
                    font=("Arial", 13), bg=theme['card'], fg=punct_color).pack(side='left', padx=10)
            
            # Total records
            total_frame = tk.Frame(content_frame, bg=theme['card'])
            total_frame.pack(fill='x', pady=10)
            tk.Label(total_frame, text="📈 Total Records:", font=("Arial", 13, "bold"), 
                    bg=theme['card'], fg=theme['fg'], width=15, anchor='w').pack(side='left')
            tk.Label(total_frame, text=f"{len(sign_in_times)} days", 
                    font=("Arial", 13), bg=theme['card'], fg=theme['fg']).pack(side='left', padx=10)
        
    except Exception as e:
        error_label = tk.Label(frame, text=f"Error loading work pattern data: {str(e)}", 
                              bg=theme["bg"], fg="red", font=("Arial", 12))
        error_label.pack(pady=50)
...
def show_productivity_score(frame, emp_id, cursor):
    ...
    tk.Label(frame, text="🎯 Detailed Productivity Analysis", 
            font=("Arial", 18, "bold"), bg=theme["bg"], fg=theme["fg"]).pack(pady=15)
    ...
    try:
        ...
        if not row or row[0] == 0:
            tk.Label(frame, text="No attendance records found for productivity analysis", 
                    bg=theme["bg"], fg=theme["fg"], font=("Arial", 12)).pack(pady=50)
            return
        ...
        attendance_score = min(100, (total_days / 20) * 100)  # Based on 20 days as full score
        ...
        tk.Label(left_frame, text=f"{productivity_score:.1f}", font=("Arial", 50, "bold"), 
                bg=theme['card'], fg=theme['button']).pack(pady=(30, 5))
        tk.Label(left_frame, text="/100", font=("Arial", 16), 
                bg=theme['card'], fg=theme['fg']).pack()
        tk.Label(left_frame, text="Overall Productivity", font=("Arial", 12), 
                bg=theme['card'], fg=theme['fg']).pack(pady=5)
        tk.Label(left_frame, text=f"Grade: {grade}", font=("Arial", 20, "bold"), 
        ...
        tk.Label(details_frame, text="📊 Detailed Metrics", font=("Arial", 14, "bold"), 
                bg=theme['card'], fg=theme['fg']).pack(anchor='w', pady=(0, 15))
        
        # Punctuality metric
        punct_frame = tk.Frame(details_frame, bg=theme['card'])
        punct_frame.pack(fill='x', pady=8)
        tk.Label(punct_frame, text="⏰ Punctuality Score", font=("Arial", 11, "bold"), 
                bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        tk.Label(punct_frame, text=f"{punctuality_score:.1f}%", font=("Arial", 14, "bold"), 
                bg=theme['card'], fg=theme['button']).pack(anchor='w')
        tk.Label(punct_frame, text=f"{on_time_days} on-time, {late_days} late", font=("Arial", 9), 
                bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        
        # Attendance metric
        att_frame = tk.Frame(details_frame, bg=theme['card'])
        att_frame.pack(fill='x', pady=8)
        tk.Label(att_frame, text="📅 Attendance Score", font=("Arial", 11, "bold"), 
                bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        tk.Label(att_frame, text=f"{attendance_score:.1f}%", font=("Arial", 14, "bold"), 
                bg=theme['card'], fg="#66BB6A").pack(anchor='w')
        tk.Label(att_frame, text=f"{total_days} total days recorded", font=("Arial", 9), 
                bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        
        # Work hours metric
        hours_frame = tk.Frame(details_frame, bg=theme['card'])
        hours_frame.pack(fill='x', pady=8)
        tk.Label(hours_frame, text="⏱️ Work Hours Score", font=("Arial", 11, "bold"), 
                bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        tk.Label(hours_frame, text=f"{work_hours_score:.1f}%", font=("Arial", 14, "bold"), 
                bg=theme['card'], fg="#FFA726").pack(anchor='w')
        hours_text = f"{avg_hours:.1f} hrs/day average" if avg_hours else "No complete days"
        tk.Label(hours_frame, text=hours_text, font=("Arial", 9), 
                bg=theme['card'], fg=theme['fg']).pack(anchor='w')
        
    except Exception as e:
        error_label = tk.Label(frame, text=f"Error calculating productivity score: {str(e)}", 
                              bg=theme["bg"], fg="red", font=("Arial", 12))
        error_label.pack(pady=50)
